...
async function submitApprovalDecision(
  ...
  if (!response.ok) throw new Error(`审批请求失败（HTTP ${response.status}）`)
...
export async function approveDecision(
  ...
  return submitApprovalDecision(decisionId, 'approve', body, credential)
}
...
export async function denyDecision(
  ...
  return submitApprovalDecision(decisionId, 'deny', body, credential)
}
