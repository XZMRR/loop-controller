...
func TestDelegationIdempotencyEquivalentJSONReturnsOriginalResponse(t *testing.T) {
	_, server := newTestServerWithMockR2(t, true)
	registerExecutor(t, server)
	first := []byte(`{"request_id":"req-idempotency-equivalent","initiator_agent_id":"planner","target_agent_id":"executor","tool_name":"echo","arguments":{"x":"hello","nested":{"a":1,"b":2}},"session_id":"","task_id":"","risk_level":"","protocol_version":"0.42.0"}`)
	second := []byte(`{
		"protocol_version":"0.42.0", "risk_level":"", "task_id":"", "session_id":"",
