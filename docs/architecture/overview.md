...
| **R1 Agent** | 业务部门/一线员工 | 接收任务、自检、生成动作申报、接收 R2 授权后的执行结果并返回 | 实时 |
...
| **ActionProposal** | R1 向 R2 申报的拟执行动作 | 业务申请 |
| **ToolCall** | 经 R2 授权后由 MCP Gateway 转发到外部系统的真实调用 | 业务操作 |
| ~~**Action**~~ | *早期抽象，已拆分为 ActionProposal（R1 申报）与 ToolCall（R2 授权后的外部调用）* | — |
...
2. ~~**R3 审计结论强制力**：R3 发现严重违规时能否触发暂停/降级？~~  
   **已收敛**：R3 只有建议权，无暂停/降级/修改配置等实操权限，发现严重问题只能向 R0 报告。
3. ~~**R4 定义**：讨论中提到的 R0-R4 中，R4 指什么？~~  
   **已收敛**：R4 为讨论中的误说法，架构保持 **R0-R3 + 基础设施层**，详见 [00_r0r3_architecture.md](./00_r0r3_architecture.md)。
